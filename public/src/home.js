...
function getBooksBorrowedCount(books) {
  ...
  function getBooksBorrowedCount(books) // This is the helper function I used to support this task.
  {
    const listBooks = books.filter((book) => !book.borrows[0].returned);
    let count = 0;
    function bookCount(listBooks)
    {
      for(servey in listBooks)
      {
        count++;
      }
      return count;
    }
    return bookCount(listBooks);
